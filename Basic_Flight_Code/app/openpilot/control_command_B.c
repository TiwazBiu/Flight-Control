...
//  stabDesired.Throttle=(Nrf_in_switch[2])/30.0 + auto_throttle;
  ...
  if(stablization_mode)
  {
    if(control_flag>0)
    {
      stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + angle_x_out;//- Pic_cotrol_xout+fix_pitch;//control_x_out; //+ angle_x_out ;
      stabDesired.Roll=(Nrf_in_switch[1])/30.0 + angle_y_out;//+ Pic_cotrol_yout-fix_roll;//control_y_out; //+ angle_y_out ;

      control_flag--;
    }
    else if(control_flag==0)
    {
      stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + angle_x_out;//- Pic_x_out+fix_pitch;//control_x_out; //+ angle_x_out ;
      stabDesired.Roll=(Nrf_in_switch[1])/30.0 + angle_y_out;//+ Pic_y_out-fix_roll;//control_y_out; //+ angle_y_out ;     
    }
  }
  else if(!stablization_mode)
  {
    stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + angle_x_out;//+ control_x_out+fix_pitch; //+ angle_x_out ;
    stabDesired.Roll=(Nrf_in_switch[1])/30.0 + angle_y_out;//+ control_y_out-fix_roll; //+ angle_y_out ;
...
void command_handler_normal(void)
{
...
/* approximately 2~105; seems these value are all mannually bound within 0~100                */
  ...
  if(control_flag>0)
    {
      stabDesired.Pitch=(Nrf_in_switch[0])/30.0+ Pic_cotrol_yout;//+fix_pitch;//control_x_out; //+ angle_x_out ;
      stabDesired.Roll=(Nrf_in_switch[1])/30.0+ Pic_cotrol_xout;//-fix_roll;//control_y_out; //+ angle_y_out ;
      //stabDesired.Yaw=Pic_cotrol_yaw;
      control_flag--;
    }
    else if(control_flag==0)
    {
      stabDesired.Pitch=(Nrf_in_switch[0])/30.0+ Pic_y_out;//+fix_pitch;//control_x_out; //+ angle_x_out ;
      stabDesired.Roll=(Nrf_in_switch[1])/30.0+ Pic_x_out;//-fix_roll;//control_y_out; //+ angle_y_out ;     
    }
  //-----------------------------------
  //--------------------------光流模式
  //stabDesired.Pitch=(Nrf_in_switch[0])/30.0+ angle_x_out+UART_PIDadjust.fix_pitch;// - Pic_cotrol_xout+fix_pitch;//control_x_out; //+ angle_x_out ;
  //stabDesired.Roll=(Nrf_in_switch[1])/30.0+ angle_y_out+UART_PIDadjust.fix_roll;// + Pic_cotrol_yout-fix_roll;//control_y_out; //+ angle_y_out ;
  //----------------------------------------
...
//void flow_stay()
//{
//  stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + angle_x_out+fix_pitch;
//  stabDesired.Roll=(Nrf_in_switch[1])/30.0 + angle_y_out-fix_roll;
//
//}
///*
//========================================================================================================================
...
//void final_stay()
//{
//  stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + pic_x_out + fix_pitch;
//  stabDesired.Roll=(Nrf_in_switch[1])/30.0 + pic_y_out - fix_roll;
//}
//
///*
//========================================================================================================================
...
//  stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + angle_x_out + fix_pitch;
//  stabDesired.Roll=(Nrf_in_switch[1])/30.0 + angle_y_out - fix_roll;
//  yaw_temp=(Nrf_in_switch[3])/30.0 + auto_yaw;
//  if(fabs(yaw_temp)>5)
//    stabDesired.Yaw=yaw_temp;
//  else
//    stabDesired.Yaw=Yaw_fixed;
//
//  if(fabs(yaw_temp)<=5 && fabs(Yaw_con_before)>5)
//    Yaw_fixed=attitudeActual.Yaw;
//  
//  Yaw_con_before=yaw_temp;
//  
//}
//
///*
...
//void go_to_flow()
//{
//  stabDesired.Pitch=(Nrf_in_switch[0])/30.0 + control_x_out + fix_pitch;
//  stabDesired.Roll=(Nrf_in_switch[1])/30.0 + control_y_out- fix_roll;  
//}
//
///*
//========================================================================================================================
